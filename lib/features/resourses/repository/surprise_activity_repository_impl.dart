...
class SurpriseActivityRepositoryImpl implements SurpriseActivityRepository {
  ...
  Future<void> moverAPapelera(int id) async {
    ...
    await db.update(
      DatabaseConfig.tableSurpriseActivities,
      {
        ...
        DatabaseConfig.colSurpriseActivityDeletedAt: DateTime.now()
            .toIso8601String(),
      },
      where: '${DatabaseConfig.colSurpriseActivityId} = ?',
      whereArgs: [id],
  ...
  Future<void> limpiarPapeleraExpirada() async {
    ...
    await db.delete(
      DatabaseConfig.tableSurpriseActivities,
      where:
          '${DatabaseConfig.colSurpriseActivityDeletedAt} IS NOT NULL AND ${DatabaseConfig.colSurpriseActivityDeletedAt} < ?',
      whereArgs: [limite],
    );
  }
}
